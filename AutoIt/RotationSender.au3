...
; Stored as "HEX|KEY" pairs to stay compatible with AutoIt array initialization limits.
Global Const $COLOR_MAP[36] = [ _
    "FF0000|0", _ ; Red -> 0
    "00FF00|1", _ ; Green -> 1
    "0000FF|2", _ ; Blue -> 2
    "FFFF00|3", _ ; Yellow -> 3
...
Func _GetKeyForColor($iColor)
    ...
    For $i = 0 To UBound($COLOR_MAP) - 1
        Local $sEntry = $COLOR_MAP[$i]
        Local $iDelimiter = StringInStr($sEntry, "|")
        If $iDelimiter = 0 Then
            ContinueLoop
        EndIf
        
        If StringLeft($sEntry, $iDelimiter - 1) = $sHexColor Then
            Return StringMid($sEntry, $iDelimiter + 1)
        EndIf
    Next
